...
try:
    ...
    # コンテキスト残量チェック（statusline.ts が書き出した値を読む）
    ctx_line = ''
    try:
        with open('/tmp/context_usage.json') as cf:
            ctx = json.load(cf)
        used = ctx.get('used_percentage', 0)
        parts.append(f\"context={used}%\")
        if used >= 80:
            ctx_line = '[コンテキストが残り少ない。今のうちに重要な記憶を保存すること]'
        elif used >= 60:
            ctx_line = '[コンテキスト半分以上消費。長くなりそうなら記憶の保存を意識すること]'
    except Exception:
        pass
